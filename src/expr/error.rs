...
impl std::fmt::Display for ParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ParseError::UnterminatedQuote { position, quote_char } => {
                write!(f, "Unterminated quote '{}' at position {}", quote_char, position)
            }
            ...
            ParseError::MissingValue { key, position } => {
                write!(f, "Missing value for key '{}' at position {}", key, position)
            }
